...
public class MessageManager {
    ...
    public static void createMessageFile() {
        if (!(getMessagesFile().exists())) {
            ...
            msg.addDefault("Notifications.Arena.Full", "&4You cannot join that arena because it is full!");
            msg.addDefault("Notifications.Arena.State.Starting", "&4You cannot join that arena because it already starting!");
            msg.addDefault("Notifications.Arena.State.InGame", "&4You cannot join that arena because it is already in game!");
            msg.addDefault("Notifications.Arena.State.Regenerating", "&You cannot join that arena while it is regenerating!");
            msg.addDefault("Notifications.Arena.Spectate.Cannot", "&4That arena is currently not spectatable!");
            ...
            msg.addDefault("Notifications.Int.Arena", "&4Arena Id must be a valid Integer (EX: 1, 2, 3)!");
